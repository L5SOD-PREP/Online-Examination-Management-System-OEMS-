        ...
        <div className="p-8">
          ...
          {showForm && (
            <div className="bg-white rounded-lg shadow-md p-6 mb-8 border-l-4 border-blue-500">
              <h3 className="text-xl font-bold text-gray-800 mb-4">{editingExamId ? 'Edit Exam' : 'Create New Exam'}</h3>
              <form onSubmit={handleCreateOrUpdateExam} className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div>
                    <label className="block text-gray-700 font-medium mb-1">Passing Marks</label>
                    <input
                      type="number"
                      required
                      min="1"
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {exams.map((exam) => (
                    <tr key={exam.exam_id}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-500">{exam.duration} mins</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-500">{exam.total_marks}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                           exam.passing_marks <= (exam.total_marks / 2) ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'
                        }`}>
                          {exam.passing_marks}
                        </span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <button onClick={() => handleEditClick(exam)} className="text-indigo-600 hover:text-indigo-900 mr-4">Edit</button>
                        <button onClick={() => navigate('/questions', { state: { examId: exam.exam_id } })} className="text-blue-600 hover:text-blue-900 mr-4">Manage Questions</button>
